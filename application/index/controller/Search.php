...
class Search extends Controller
{
...

    {
        ...
        if (!empty($_FILES['file'])) {
            ...
            if (!isset($result['result'])) {
                ...
                $search = tridb(preg_replace($pattern, ' ', $search)); //去掉两边空格和所有标点

                $re = db('api_info')->where('keywords like "%' . $search . '%" or title="' . $search . '"')->select();
    ...
    public function add_api_num($name, $openid = "", $max_num = 500)
    {
        ...
        $api_arr = db('api_info')->where(['api_name'=>$name])->find();

        if (empty($api_arr)) {
            return false;
        } else {
           $re = db('api_info')->where(['api_name'=>$name])->setInc('num');
       
        }
        if ($openid && $openid != "null") {
            $re = db('interface2')->where(array('openid' => $openid))->find();
            ...
            if (empty($re)) {
                ...
                $re = db('interface2')->where(array('openid' => $openid))->update(array('info' => json_encode($info)));
            }
        }
    }
     public function add_api_num2($name, $openid = "")
    {
        ...
        $api_arr = db('api_info')->where(array('api_name' => $name))->find();

        if (empty($api_arr)) {
            return false;
        } else {
            db('api_info')->where(array('api_name' => $name))->setInc('num');
        }
        if ($openid && $openid != "null") {
            $data['openid']    = $openid;
            $data['api_id']    = $api_arr['id'];
            $data['date_time'] = date('Ymd');
            $re                = db('interface')->where($data)->find();

            if (empty($re)) {
                $data['num']       = 1;
                $data['date_time'] = date('Ymd');
                db('interface')->add($data);
            } else {
                if ($re['num'] < 500) {
                    db('interface')->where($data)->setInc('num');
                } else {
                    apiResponse("0", "当天可调用次数已用完");
                }
            }
